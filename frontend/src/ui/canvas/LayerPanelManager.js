...
class LayerPanelManager {
  ...
  showLayerPanel(nodeId) {
    ...
    const formattedType = typeParts.map(part => part.charAt(0).toUpperCase() + part.slice(1)).join(' ');
    ...
    const basicParamsContainer = DomUtils.createElementWithClass('div', 'basic-params-container');
    ...
    if (hasAdditionalParams) {
      additionalParamsContainer = DomUtils.createElementWithClass('div', 'additional-params-container');
      ...
      additionalParamsContent = DomUtils.createElementWithClass('div', 'additional-params-content');
      additionalParamsContent.style.maxHeight = this.showAdditionalParams ? '1000px' : '0';
      additionalParamsContent.style.opacity = this.showAdditionalParams ? '1' : '0';
      ...
      additionalHeader.querySelector('button').addEventListener('click', () => {
        ...
        additionalParamsContent.style.maxHeight = this.showAdditionalParams ? '1000px' : '0';
        additionalParamsContent.style.opacity = this.showAdditionalParams ? '1' : '0';
        additionalHeader.querySelector('button').textContent = this.showAdditionalParams ? 'Show Less' : 'Show More';
      });
    }
    ...
    for (const key in params) {
      if (Object.prototype.hasOwnProperty.call(params, key)) {
        const paramDef = paramDefinitions[key];
        const paramControl = this.createParameterControl(key, params[key], layer, paramDef);
