...
class LayerModel {
  ...
  static VISUAL_PARAMETERS = {
    ...
    EmbeddingLayer: ['input_dim', 'output_dim', 'embeddings_initializer', 'embeddings_regularizer', 'embeddings_constraint', 'mask_zero', 'lora_rank']
  };
  ...
  updateDropoutLayerVisualization() {
    ...
    if (svgElement) {
      ...
      if (!probabilityText) {
        probabilityText = document.createElementNS('http://www.w3.org/2000/svg', 'text');
  ...
  updateEmbeddingLayerVisualization() {
    ...
    if (svgElement) {
      ...
      if (!dimensionsText) {
        dimensionsText = document.createElementNS('http://www.w3.org/2000/svg', 'text');
