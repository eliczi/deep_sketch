...
class LayerFactory {
  ...
  static _populateNodeContent(node, type, layerTypeDef) {
    ...
    if (layerTypeDef.name === "ConvolutionalLayer") {
      const svgRepresentation =
        SVGGenerator.createSVGRepresentation("ConvolutionalLayer");
      ...
      layerTypeDef &&
      layerTypeDef.svg_representation &&
      layerTypeDef.svg_representation.svg_content
    ) {
  ...
  static _setInputLayerSVG(svgContainer, layerTypeDef) {
    if (layerTypeDef && layerTypeDef.svg_representation) {
      const inputType = layerTypeDef.name
        .replace("InputLayer", "")
        .toUpperCase();
      if (
        inputType &&
  ...
  static makeDraggable(element) {
    ...
    function getCanvasInfo(canvas) {
      ...
      const zoomFloat = zoomIndicator
        ? parseFloat(zoomIndicator.textContent)
        : 100;
    ...
    function updateAttachedFunctionLayers(
      ...
      attachedFunctionLayers.forEach((functionLayer) => {
        ...
        if (functionLayer.dataset.groupId) {
          ...
          functionLeft =
            transformedX + element.offsetWidth * scale - 22 * scale;
