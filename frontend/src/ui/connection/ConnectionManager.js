...
/**
 * Manages the creation and management of connection lines between nodes on the canvas.
 * Handles user interaction for drawing and editing connections.
 * @class
 */
export class ConnectionManager {
  ...
  findSourceAndTargetNodes(sourceId, targetId) {
    ...
    if (sourceGroupId && targetGroupId && sourceGroupId === targetGroupId) {
      const groupElement = document.querySelector(
        `.layer-group[data-id="${sourceGroupId}"]`,
      );
  ...
  updateConnection(connectionElement, sourceId, targetId) {
    ...
    if (result?.missingNodes || result?.missingPoints) {
      console.warn(
        `Problem with connection: ${result.sourceId} -> ${result.targetId}`,
      );
      return;
    }
  ...
  updateAllConnections() {
    if (
      !this.networkModel.connections ||
      this.networkModel.connections.length === 0
    ) {
      return;
    }
  ...
  removeConnectionsForNode(nodeId) {
    const connectionsToRemove = this.networkModel.connections.filter(
    ...
    this.networkModel.connections = this.networkModel.connections.filter(
      (conn) => conn.sourceId != nodeId && conn.targetId != nodeId,
    );
  }
