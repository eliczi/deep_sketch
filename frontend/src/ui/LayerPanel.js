...
class LayerPanel {
  ...
  renderLayerTypes(layerTypes) {
    ...
    const scrollableWrapper = DomUtils.createElementWithClass('div', 'layer-panel-scrollable');
    ...
    Object.entries(categories).forEach(([categoryName, categoryItems]) => {
      ...
      categoryItems.forEach(layerType => {
        ...
        const iconContainer = DomUtils.createElementWithClass('div', 'layer-icon');
        if (layerType.svg_representation && layerType.svg_representation.svg_content) {
          iconContainer.innerHTML = layerType.svg_representation.svg_content;
        } else {
          iconContainer.innerHTML = '<svg width="32" height="32"><rect width="32" height="32" fill="#eee"/></svg>';
        }
